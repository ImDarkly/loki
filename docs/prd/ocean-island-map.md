...
## Solution
The lake becomes a large, effectively boundless ocean surrounding a small circular island at the center, sized to contain today's existing spawn points, shop hut, and storage box (the shop hut being the one entity relocated onto the island — see Decision 3 and User Story 1). The island sits visibly above the ocean's surface with a hard shoreline edge; players are physically confined to it, and walking off the edge drops them into the ocean, which is treated as a lethal fall that routes through the game's existing death/respawn flow — the same one the shark attack already uses. Fishing zones and the shark's spawn/patrol behavior keep operating at exactly the same range from shore as they do today, so day-one fishing feel doesn't change, only the world around it. All of the map's geometry (island size, fishable range, ocean extent, and center point) is consolidated into one small shared config that the four affected systems read from, instead of each keeping its own independent copy.
## User Stories
...
7. As a player, I want my existing spawn point and storage box to still be exactly where they are today (the shop hut being the sole exception, relocated per User Story 1), so that this map change doesn't disorient me or break my mental map of the space.
...
- **RPC mode is `"authority"`, not `"any_peer"`** (Decision 13). The fall report is invoked only by the node's own authority (owner client), so `"authority"` scopes it correctly and blocks any peer from remotely killing another player's node. Functionally equivalent to the spec for legitimate play; the stricter mode is deliberate and documented in `player.gd`.
- **`_respawn_at_spawn()` runs on every `_on_restart`.** Pre-existing restart never repositioned the player; without this, a fall-death in the ocean would re-trigger the fall check the instant the round restarts. Behavior change for all deaths (including existing shark deaths) — players now respawn at their spawn point on round restart.
- **Spectate bodies freeze at the waterline.** Once below `FALL_DEATH_Y` in spectate, the body stops moving so a fallen corpse doesn't sink into the void.
- **Fishing zones sit in a water ring** (`ISLAND_RADIUS + zone_radius + margin` to `FISHABLE_BAND_RADIUS - zone_radius - margin`) rather than the full fishable-band disc, because the island now occupies the center. Range from shore is preserved; this is an unplanned-but-necessary consequence of the island.
- **Rock placement samples to `ISLAND_RADIUS - ROCK_HALF_EXTENT`** (and `_is_on_land` validates against the same) so rock centers don't overhang the shoreline — a small refinement on top of Decision 7's single island-radius check.