...
func test_escape_feedback_transitions_to_idle() -> void:
	mechanic._enter_reeling()
	mechanic.reel_timer.stop()
	mechanic.reel_meter.size = Vector2(40, 300)
	mechanic.player_bar_position = 90.0
	...
	mechanic.quota = 0
	mechanic._on_reel_timer_timeout()
	assert_eq(mechanic.current_state, 5, "Should be ESCAPE (5) after reel timeout")
	mechanic._on_catch_feedback_completed()
	assert_eq(mechanic.current_state, 0, "Should be IDLE (0) after escape feedback completes")
