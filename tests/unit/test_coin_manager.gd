...
func test_shark_bait_late_join_visibility() -> void:
	...
	assert_true(player.holding_shark_bait, "Late-joining player should hold shark bait if already owned")
	assert_not_null(player._held_bait_mesh, "Late-joining player should have held bait mesh")
	assert_false(player._rod_pivot.visible, "Late-joining player should have rod hidden")
	players.queue_free()
	await get_tree().process_frame
