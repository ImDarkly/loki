...
func _ready():
	...
	print("SPEAKER: init parent_playing=%s stream=%s" % [get_parent().playing, audiostreamopus != null])
...
func setrecopusvalues(opus_sample_rate, opus_channels):
	if audiostreamplaybackopus == null or not get_parent().playing or audiostreamopus.opus_sample_rate != opus_sample_rate or audiostreamopus.opus_channels != opus_channels:
		...
		print("SPEAKER: setrecopus -> play() playback=%s" % (audiostreamplaybackopus != null))
...
func unpausewhenbufferready():
	...
	if bufferlengthtime > audiobufferlagtimetarget:
		print("SPEAKER: unpause buffer_time=%.3f" % bufferlengthtime)
	...
	if len(packet) <= 3:
		...
		if h != null:
			...
			if h.has("talkingtimestart"):
				print("SPEAKER: header sr=%d ch=%d framesize=%d" % [h["opussamplerate"], h.get("opuschannels", 2), h.get("opusframesize", -1)])
		...
		if opusframecountR >= 0:
			while opusframecountR >= Noutoforderqueue:
				...
				if outoforderchunkqueue[0] != null:
					print("SPEAKER: push frame (shift) count=%d" % opusframecount)
			...
			while outoforderchunkqueue[0] != null and opusframecount + opusframequeuecount >= Npacketinitialbatching:
				if opusframesize > audiostreamplaybackopus.available_space_frames():
					...
					break
					print("SPEAKER: push frame (batch) count=%d" % opusframecount)
