...
func _ready() -> void:
	...
	if _label:
		_label.text = "Dev Shark Bait Flow — C buy / P place water / I try island / O toggle owned / R reset / B +5 coins"
...
func _place_at_camera() -> void:
	...
	if _bait_manager.is_placed and not was_placed:
		_last_place_result = "Placed at (%.1f, %.1f) ✓" % [target.x, target.z]
...
func _try_place_inside() -> void:
	...
	if _bait_manager.is_placed and not was_placed:
		...
		_last_place_result = "Inside (%.1f, %.1f) correctly rejected ✓" % [inside.x, inside.z]
